...
const setupCelebrations = (sock, gid) => {
  ...
  cron.schedule('0 0 25 12 *', async () => {
    await sendMsg(sock, gid, '🎄 *MERRY CHRISTMAS!* 🎄\n\nWishing you and your family joy and blessings! 🎁⭐');
  }, { timezone: CONFIG.timezone });
...
• Invite gate (add ${CONFIG.inviteGate} people to chat)`);
      break;

    case 'ai':
      if (!rest) return sendMsg(sock, gid, '❌ Usage: !ai your question here');
    ...
    case 'roast':
      ...
      const roast = await askAI('Give a funny harmless roast for ' + roastTarget + '. Keep it light and not offensive.');
      await sock.sendMessage(gid, { text: '🔥 @' + roastTarget + ' ' + roast, mentions: mentioned });
      break;

    case 'ship':
      if (mentioned.length < 2) return sendMsg(sock, gid, '❌ Tag 2 people! !ship @user1 @user2');
      const s1 = mentioned[0].split('@')[0];
      const s2 = mentioned[1].split('@')[0];
    ...
    case 'kick':
      ...
      if (!mentioned.length) return sendMsg(sock, gid, '❌ Tag someone to kick!');
    ...
    case 'ban':
      if (!admin) return sendMsg(sock, gid, '❌ Admins only!');
      if (!mentioned.length) return sendMsg(sock, gid, '❌ Tag someone to ban!');
      await sock.groupParticipantsUpdate(gid, mentioned, 'remove');
    ...
    case 'warn':
      if (!admin) return sendMsg(sock, gid, '❌ Admins only!');
      if (!mentioned.length) return sendMsg(sock, gid, '❌ Tag someone to warn!');
...
⚠️ Violations = warnings then kick!`);
    ...
    case 'poll':
      if (!rest) return sendMsg(sock, gid, '❌ Usage: !poll Question? Option1, Option2, Option3');
      const pp = rest.split('?');
      const pq = pp[0] + '?';
    ...
    case 'remind':
      if (!rest) return sendMsg(sock, gid, '❌ Usage: !remind 5 take your medicine');
      const rp = rest.split(' ');
      const rm = parseInt(rp.shift());
      const rt = rp.join(' ');
      if (isNaN(rm)) return sendMsg(sock, gid, '❌ First must be number of minutes. !remind 5 message');
    ...
    case 'song':
      ...
      exec('yt-dlp -x --audio-format mp3 -o "/tmp/%(title)s.%(ext)s" "ytsearch1:' + rest + '"', async (err) => {
        if (err) return sendMsg(sock, gid, '❌ Download failed. Try another song name.');
    ...
    case 'tiktok':
      if (!rest) return sendMsg(sock, gid, '❌ Usage: !tiktok [tiktok url]');
...
const startBot = async () => {
  ...
  // Welcome new members + invite gate notice
  sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
    if (action === 'add') {
      ...
      for (const p of participants) {
        ...
        await sendMsg(sock, id,
          '👋 Welcome @' + p.split('@')[0] + '!\n\n' +
          'To unlock chatting in this group you must *add ' + CONFIG.inviteGate + ' members* first!\n\n' +
          'Current: ' + inviteCount[p] + '/' + CONFIG.inviteGate + '\n\n' +
    ...
    // Track when someone adds members — credit them
    if (action === 'add' && participants.length > 0) {
      // This is simplified — full invite tracking needs link-based detection
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        if (!botAdmin) continue; // Skip if bot is not admin
        ...
        if (!admin && !approvedUsers.has(sender)) {
          ...
          if (cnt < CONFIG.inviteGate) {
            ...
            await sendMsg(sock, gid,
              '⛔ @' + sender.split('@')[0] + ' you need to add *' + (CONFIG.inviteGate - cnt) + ' more members* before chatting!\n(' + cnt + '/' + CONFIG.inviteGate + ')',
              [sender]
            );
            continue;
          } else {
        ...
        // Setup celebrations once per group
