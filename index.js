...
const saveSessionToEnv = async () => {
  try {
    ...
    // Save only creds.json — it's the only file needed to avoid re-pairing.
    // Saving all session files exceeds Railway's 32768 char variable limit.
    const credsPath = dir + '/creds.json';
    if (!fs.existsSync(credsPath)) return;
    const files  = { 'creds.json': fs.readFileSync(credsPath, 'utf8') };
...
const sendMsg = async (sock, jid, text, mentions) => {
  // Guard: never send empty or non-string messages — this causes blank message spam
  if (!text || typeof text !== 'string' || !text.trim()) {
    process.stdout.write('sendMsg blocked: empty text\n');
    return;
  }
  try {
...
const handleCmd = async (sock, msg, text, gid, sender) => {
  try {
    // botAdmin already confirmed in messages.upsert before calling here
    ...
    process.stdout.write('[CMD] cmd=' + cmd + ' sender=' + sender + ' gid=' + gid + '\n');
...
⚠️ Violations = warnings then kick!`);
    ...
    process.stdout.write('[CMD ERROR] cmd failed: ' + e.stack + '\n');
    await sendMsg(sock, gid, '❌ An error occurred processing that command.');
  }
};
...
📌 Refer to the group's description for group rules.

🚫 *No spamming. No scamming.*

🤝 Make sure the person you're dealing with in the group is *trusted* before any transactions.
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        try {
          ...
          // Extract text from all common message types including ephemeral and viewOnce
          const text =
          ...
          // ── Commands run regardless of botAdmin so !menu always works ──
          if (text && text.startsWith(CONFIG.prefix)) {
            if (!botAdmin) {
              process.stdout.write('[CMD] blocked — bot is not admin in ' + gid + '\n');
              await sendMsg(sock, gid, '❌ Bot must be group admin to process commands.');
              continue;
            }
